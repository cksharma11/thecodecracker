export const systemDesignQuestions = [
  {
    ...
    difficulty: "Easy",
    summary: "A high-performance service to convert long URLs into short, unique aliases, redirecting users upon access.",
    requirements: {
      functional: [
        "Generate a unique short alias (7 characters) for a given long URL.",
        "Redirect users visiting the short URL to the original long URL with a 302 HTTP status.",
        "Support custom aliases (e.g., tinyurl.com/myCustomLink) with check-and-insert validation."
      ],
      nonFunctional: [
        "High availability for redirection path (99.999% uptime target).",
        "Redirection latency under 30ms via aggressive caching.",
        "Ensure short links are random and non-predictable to prevent enumeration attacks."
      ]
    },
    estimations: "100M URLs generated per month (3 million per day). Read/Write ratio: 100:1. Write QPS: ~40. Read QPS: ~4,000. Storage (5 years): 6 billion URLs * 500 bytes = 3TB. Bandwidth: Ingress ~20KB/s, Egress ~2MB/s.",
    apis: [
      { method: "POST", path: "/api/v1/urls", desc: "Shorten a URL", request: "{ \"longUrl\": \"https://example.com/very/long/path\", \"customAlias\": \"string?\" }", response: "{ \"shortUrl\": \"https://tinyurl.com/aB34efG\" }" },
      { method: "GET", path: "/{shortKey}", desc: "Redirect to target URL", request: "None", response: "302 Redirect Location Header" }
    ],
    dataModel: {
      type: "NoSQL Wide-Column DB (Cassandra) for scalability and partition throughput",
      schema: "URLMap Table:\n- shortKey (varchar, Partition Key): Primary index hash\n- originalUrl (varchar): Destination URL\n- userId (varchar): Owner ID for analytics\n- createdAt (timestamp): Insertion date"
    },
    highLevelFlow: [
      { from: "Client", to: "Load Balancer", label: "GET /{shortKey}" },
      { from: "Load Balancer", to: "App Servers", label: "Route Request" },
      { from: "App Servers", to: "Redis Cache", label: "Query shortKey" },
      { from: "App Servers", to: "Cassandra DB", label: "Cache Miss: Lookup in DB and backfill cache" }
    ],
    deepDive: "Uses Base62 encoding (a-z, A-Z, 0-9) to convert a 64-bit auto-incrementing unique ID (generated by a distributed Snowflake ID generator) into a 7-character string. Database partition keys are set on the 'shortKey' to distribute reads uniformly. A cluster of Redis nodes running an LRU eviction policy caches hot redirects, absorbing 98%+ of read traffic and keeping redirect latencies under 30ms.",
    tradeoffs: "eventual consistency vs ACID: Relational DBs are bypassed because URL mappings are immutable; we don't need complex transactions. Cassandra's write performance and simple primary key lookups scale linearly. 302 redirects are used instead of 301 to ensure redirect traffic passes through our servers for real-time analytics collection."
  },
  {
    id: 2,
    ...
    difficulty: "Medium",
    summary: "A security service to throttle incoming traffic to web applications to prevent DDoS attacks, API abuse, and brute force attempts.",
    requirements: {
      functional: [
        "Throttle requests per user, IP, or API token based on predefined limits (e.g., 100 requests/min).",
        "Block excess requests and return standard 429 Too Many Requests status with Retry-After headers."
      ],
      nonFunctional: [
        "Low latency footprint (< 2ms overhead per check) to prevent delaying backend requests.",
        "High accuracy across multiple distributed application nodes.",
        "Fail-open resiliency: If the rate limiting service fails, backend API traffic must proceed."
      ]
    },
    estimations: "10 billion daily API requests. Average QPS: ~115,000. Peak QPS: ~250,000. Memory: 10M active users * 32 bytes (Key + Counter) = ~320MB RAM in Redis.",
    apis: [
      { method: "Middleware", path: "Intercepts all requests", desc: "Verifies token limits", request: "Headers (Client IP / JWT / API Key)", response: "429 Limit Exceeded or 200 OK" }
    ],
    dataModel: {
      type: "In-Memory Key-Value Store (Redis)",
      schema: "RateLimit Hash:\n- key: String (e.g., rate:user_id:12345)\n- value: Hash of { 'tokens': integer, 'last_updated': timestamp }\n- TTL: Expiry set to window size"
    },
    highLevelFlow: [
      { from: "Client", to: "API Gateway", label: "HTTP API Request" },
      { from: "API Gateway", to: "Rate Limiter", label: "Check request details" },
      { from: "Rate Limiter", to: "Redis Cluster", label: "Run atomic Lua validation script" },
      { from: "API Gateway", to: "App Servers", label: "Passed: Forward Request" }
    ],
    deepDive: "Uses the Sliding Window Counter algorithm to track user requests across rolling intervals, avoiding the traffic spikes permitted by Fixed Window algorithms. To prevent race conditions during concurrent requests, token validations are executed atomically in Redis using Lua scripts. The system uses a 'fail-open' configuration: if Redis is unreachable, requests are logged as warnings and allowed to pass to protect system availability.",
    tradeoffs: "Token Bucket is memory-efficient but permits traffic bursts. Sliding Window Log is accurate but memory-intensive. Sliding Window Counter is selected as a compromise: it prevents boundary bursts while using minimal Redis memory."
  },
  {
    id: 3,
    ...
    difficulty: "Hard",
    summary: "A distributed system that crawls, downloads, and indexes the entire web for search engines.",
    requirements: {
      functional: [
        "Extract HTML content from seed URLs.",
        "Parse pages to extract text and new hyperlinks, queueing discovered links for future crawls.",
        "Detect duplicate pages and avoid recrawling them."
      ],
      nonFunctional: [
        "Scalable: Handle billions of web pages.",
        "Polite: Avoid overloading target websites (respect robots.txt and implement per-host request delays).",
        "Robust: Handle crawl traps (infinite redirect loops), broken links, and non-HTML media files."
      ]
    },
    estimations: "15 billion web pages crawled monthly. Crawl rate: ~5,800 pages/second. Bandwidth: 5,800 pages * 100KB = 580MB/sec. Storage: 15B pages * 100KB = 1.5PB/month.",
    apis: [
      { method: "Internal Job", path: "Distributed Crawler", desc: "Batch process seed URLs, fetch HTML, and parse anchors", request: "Seed URLs list", response: "Parsed pages & links" }
    ],
    dataModel: {
      type: "Distributed File System (HDFS) for raw HTML + Distributed Database (HBase) for link states",
      schema: "URLRegistry:\n- urlHash (varchar, PK)\n- url (varchar)\n- status (enum: TODO|CRAWLED|FAILED)\n- lastCrawlTime (timestamp)\n- contentHash (varchar): SimHash for duplication check"
    },
    highLevelFlow: [
      { from: "URL Frontier", to: "Crawler Workers", label: "Fetch next polite URLs" },
      { from: "Crawler Workers", to: "DNS Cache", label: "Resolve domain IP (bypasses standard DNS)" },
      { from: "Crawler Workers", to: "Target Website", label: "GET /page (Robots.txt check)" },
      { from: "Crawler Workers", to: "HDFS Storage", label: "Save HTML and parse outgoing links" }
    ],
    deepDive: "The URL Frontier manages crawling queues, using a dedicated queue per domain to enforce politeness delays. It queries a DNS Cache to avoid slow DNS lookups on every request. Pages are hashed using SimHash; if the hash of parsed text matches an existing document, the page is classified as a duplicate and discarded to save processing time.",
    tradeoffs: "Breadth-First Search (BFS) is preferred over Depth-First Search (DFS) to crawl diverse domains rather than digging infinitely down a single website's sub-links. BFS prevents crawlers from getting stuck in single-site traps."
  },
  {
    id: 4,
    ...
    category: "Web Applications",
    difficulty: "Medium",
    summary: "A highly scalable multi-channel messaging platform delivering real-time SMS, email, and push notifications.",
    requirements: {
      functional: [
      ...
      nonFunctional: [
        "Real-time delivery (< 5s for transactional alerts).",
        "Reliability: Guaranteed delivery with automatic retries and failover providers.",
        "Deduplication: Prevent duplicate messages from being sent to the same user."
      ]
    },
    estimations: "50 million active users. 100 million notifications/day. Average QPS: ~1,150. Peak QPS: ~6,000. Storage: 100M logs/day * 200 bytes = 20GB/day.",
    apis: [
      { method: "POST", path: "/api/v1/notify", desc: "Trigger notification", request: "{ \"userId\": \"uuid\", \"channel\": \"EMAIL|SMS|PUSH\", \"templateId\": \"string\", \"variables\": {} }", response: "{ \"notificationId\": \"uuid\", \"status\": \"QUEUED\" }" }
    ],
    dataModel: {
      type: "Relational DB (PostgreSQL) for settings & logs + Redis Cache for templates",
      schema: "NotificationLog:\n- id (uuid, PK)\n- userId (uuid)\n- channel (varchar)\n- status (enum: PENDING|SENT|FAILED)\n- retryCount (int)\n- createdAt (timestamp)\n- deduplicationHash (varchar)"
    },
    highLevelFlow: [
      { from: "App Server", to: "Notification API", label: "POST /notify" },
      { from: "Notification API", to: "Message Queue (Kafka)", label: "Enqueue Notification Task" },
    ...
    deepDive: "Saves notification logs to PostgreSQL and pushes delivery tasks to Kafka topics. Delivery workers consume tasks and call third-party providers (APNS, Twilio, SendGrid) asynchronously. To prevent duplicate messages during system retries, workers check a Redis deduplication cache using a hash of `(userId, templateId, date)` before sending.",
    tradeoffs: "Using message queues decouples API endpoints from slow third-party providers, but introduces potential replication lag between notification logs and database reads."
  },
  {
    id: 5,
    ...
    difficulty: "Hard",
    summary: "A real-time messaging application supporting instant 1-on-1 and group chats, online status signals, and media transfers.",
    requirements: {
      functional: [
        "Deliver messages in real-time between users with read receipts.",
        "Provide real-time online/offline presence tracking.",
        "Support media file uploads and group chats (up to 1,000 members)."
      ],
      nonFunctional: [
        "Low latency: Message delivery under 100ms.",
        "Reliability: Zero message drops (messages persist in DB when recipient is offline).",
        "Security: Support end-to-end encryption (Signal protocol)."
      ]
    },
    estimations: "500 million active users. 10 billion messages daily. Average QPS: ~115,000. Peak QPS: ~250,000. Daily Storage: 10B messages * 100 bytes = 1TB/day.",
    apis: [
      { method: "WebSocket", path: "/ws/chat", desc: "Duplex channel for messages", request: "WebSocket Upgrade Header", response: "Real-time message stream" }
    ],
    dataModel: {
      type: "Wide-Column Key-Value DB (Cassandra or ScyllaDB)",
      schema: "Messages Table:\n- channelId (varchar, Partition Key)\n- messageId (uuid, PK)\n- senderId (varchar)\n- content (text)\n- status (enum: SENT|DELIVERED|READ)\n- timestamp (timestamp, Clustering Key DESC)"
    },
    highLevelFlow: [
      ...
      { from: "WebSocket Server", to: "Cassandra DB", label: "Offline: Queue to Persistent Storage" }
    ],
    deepDive: "Maintains active WebSocket connections. The Presence Service uses Redis hashes with a 30-second TTL to track online heartbeats. For group chats, a fan-out worker reads group membership and publishes messages to the queues of all online members in parallel.",
    tradeoffs: "Cassandra is chosen over relational databases because it handles high write volumes and distributes data evenly across partitions using clustering keys, which is ideal for real-time chat histories."
  },
  {
    id: 6,
    ...
    category: "Web Applications",
    difficulty: "Hard",
    summary: "A timeline engine that aggregates, compiles, and delivers chronological posts from followed accounts in real-time.",
    requirements: {
      functional: [
        "Display a chronological feed of posts from followed accounts.",
        "Allow users to publish posts containing text and media URLs.",
        "Support paginated scrolling of feed content."
      ],
      nonFunctional: [
        "Low feed generation latency (< 150ms).",
        "High write availability (posts must succeed quickly).",
        "Scalable: Handle celebrity accounts with millions of followers (prevent fan-out bottlenecks)."
      ]
    },
    estimations: "100 million active users. Followers/user: ~200. Celebrity followers: ~10M+. Post QPS: ~5,000. Feed load QPS: ~50,000.",
    apis: [
    ...
    dataModel: {
      type: "Relational DB (PostgreSQL sharded by userId) + Redis Cache Clusters",
      schema: "Post:\n- id (uuid, PK)\n- userId (uuid)\n- text (varchar)\n- createdAt (timestamp)\n\nFollow:\n- followerId (uuid)\n- followeeId (uuid)"
    },
    highLevelFlow: [
    ...
    deepDive: "Uses a Hybrid model. Active users have pre-computed timelines stored in Redis lists (Push model). When they publish a post, it is pushed to their followers' feed caches. For celebrity accounts (millions of followers), the system uses a Pull model: their posts are fetched and merged into the follower's feed on-demand, preventing write queue bottlenecks.",
    tradeoffs: "Pre-computing timelines in Redis is fast but uses significant memory. We accept eventual consistency for feed updates to protect write performance."
  },
  {
    id: 7,
    ...
    category: "Web Applications",
    difficulty: "Hard",
    summary: "A global video platform supporting uploads, multi-bitrate transcoding, search index queries, and low-latency worldwide playbacks.",
    requirements: {
      functional: [
        "Upload raw video files and index metadata.",
        "Transcode uploads into multiple resolutions (1080p, 720p, etc.) and formats (HLS/DASH).",
        "Stream video content with dynamic quality adjustments."
      ],
      nonFunctional: [
        "Sub-second playback startup time.",
        "Zero video stuttering or buffering.",
        "Highly scalable object storage for massive video uploads."
      ]
    },
    estimations: "200 million active users. 50M uploads/month. 1 video = 100MB average. Storage needed: 5PB per month. Streaming bandwidth: ~300 Tbps.",
    apis: [
  ...
  },
  {
    ...
    category: "Real-Time / Geo",
    difficulty: "Medium",
    summary: "A prefix query service returning the top 5 trending search suggestions under 10ms as the user types.",
    requirements: {
      functional: [
  ...
  },
  {
    ...
    requirements: {
      ...
      nonFunctional: [
        ...
        "Scalable: Update locations every 4 seconds for active drivers.",
